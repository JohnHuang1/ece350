nop             # Sort
nop             # Author: Jack Proudfoot
nop             
nop
init:
addi $sp, $zero, 256        # $sp ($r29) = 256
addi $27, $zero, 3840       # $27 = 3840 address for bottom of heap
addi $t0, $zero, 50 # $t0 ($r8)
addi $t1, $zero, 3 # $t1 ($r9)
sw $t1, 0($t0)  # store 3 at mem[50]
addi $t1, $zero, 1 # $t1 = 1
sw $t1, 1($t0)  # store 1 at mem[51]
addi $t1, $zero, 4 # $t1 = 4
sw $t1, 2($t0) $ # store 4 at mem[52]
addi $t1, $zero, 2 # $t1 = 2
sw $t1, 3($t0)  # store 2 at [53]
add $a0, $zero, $t0 # $a0 ($r4) = 50
j main # go to line 94 (imem 16)
malloc:                     # $a0 = number of words to allocate 
sub $27, $27, $a0           # allocate $a0 words of memory (imem 17)
blt $sp, $27, mallocep      # check for heap overflow
mallocep:
add $v0, $27, $zero # imem 19
jr $ra
buildlist:                  # $a0 = memory address of input data
sw $ra, 0($sp) # (imem 21) store line 78 into mem[256]
addi $sp, $sp, 1 # increment $sp = 257
add $t0, $a0, $zero         # index of input data $t0 = 50
add $t1, $zero, $zero       # current list pointer $t1 = 0
addi $a0, $zero, 0  # $a0 (r4) = 0
jal malloc # jal to line imem 17
addi $t3, $v0, -3           # list head pointer ($t3 = $r11)
lw $t2, 0($t0)              # load first data value ($t2 = $r10 = mem[50] = 3)
j blguard
blstart:
addi $a0, $zero, 3 # imem 30 $a0 = 3
jal malloc # $ra = 32, pc = 17
sw $t2, 0($v0)              # set new[0] = data
sw $t1, 1($v0)              # set new[1] = prev
sw $zero, 2($v0)            # set new[2] = next
sw $v0, 2($t1)              # set curr.next = new
addi $t0, $t0, 1            # increment input data index
lw $t2, 0($t0)              # load next input data value
add $t1, $zero, $v0         # set curr = new
blguard: 
bne $t2, $zero, blstart # imem 39
add $v0, $t3, $zero         # set $v0 = list head
addi $sp, $sp, -1
lw $ra, 0($sp)
jr $ra # imem 43
sort:                       # $a0 = head of list
sw $ra, 0($sp) # imem 44
addi $sp, $sp, 1
sortrecur:
addi $t7, $zero, 0          # $t7 = 0
add $t0, $a0, $zero         # $t0 = head
add $t1, $t0, $zero         # $t1 = current
j siguard
sortiter:
lw $t2, 0($t1)              # $t2 = current.data (imem 50)
lw $t3, 0($t6)              # $t3 = current.next.data
blt $t2, $t3, sinext
addi $t7, $zero, 1          # $t7 = 1
lw $t4, 1($t1)              # $t4 = current.prev
bne $t4, $zero, supprev
j supprevd
supprev:
sw $t6, 2($t4)              # current.prev.next = current.next (imem 57)
supprevd:
sw $t4, 1($t6)              # current.next.prev = current.prev
lw $t5, 2($t6)              # $t5 = current.next.next
bne $t5, $zero, supnnprev
j supnnprevd
supnnprev:
sw $t1, 1($t5)              # current.next.next.prev = current (imem 62)
supnnprevd:
sw $t5, 2($t1)              # current.next = current.next.next
sw $t1, 2($t6)              # current.next.next = current
sw $t6, 1($t1)              # current.prev = current.next
bne $t0, $t1, sinext
add $t0, $t6, $zero         # head = current.next
sinext:
add $t1, $t6, $zero         # $t1 = current.next
siguard:
lw $t6, 2($t1)              # $t6 = current.next (imem 69)
bne $t6, $zero, sortiter
add $a0, $t0, $zero
bne $t7, $zero, sortrecur
add $v0, $t0, $zero         # $v0 = head
addi $sp, $sp, -1
lw $ra, 0($sp)
jr $ra
main:
jal buildlist # (imem 77) jal to line 21
add $t0, $v0, $zero         # $t0 = head of list
add $a0, $t0, $zero         # $a0 = head of list
jal sort # $ra = 81, pc = 44
add $t0, $v0, $zero         # $t0 = head of sorted list
add $t5, $zero, $zero
add $t6, $zero, $zero
add $t1, $t0, $zero
j procguard
proclist:
lw $t2, 0($t1)
add $t5, $t5, $t2
sll $t6, $t6, 3
add $t6, $t6, $t5
lw $t1, 2($t1)
procguard:
bne $t1, $zero, proclist
stop:
nop
nop
nop
j stop